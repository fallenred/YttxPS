...
jQuery(function($) {
    var fsRoomno = $.getUrlParam('no');   //房型序号
    var fsRoomname = $.getUrlParam('name');   //房型名称

    if(fsRoomno == null || fsRoomno == "" || fsRoomno == undefined){
        alert("未取到该房型编号！");
    ...
    $(grid_selector).jqGrid({
            url : "/room/findRoomPrice.htm",
            postData:{ "roomPrice.fsResno" : fsRoomno},
            datatype : "json",
            mtype : 'POST',
            height : 395,
            ...
            colModel : [ 
                ...
                autowidth : true,
                caption: (fsRoomname != null && fsRoomname != "") ? (fsRoomname + " — 房型价格列表") : "房型价格列表"
    });
    ...
    $(grid_selector).jqGrid('navGrid', pager_selector).jqGrid('navButtonAdd',
            pager_selector, {
                ...
                onClickButton : function() {
                    ...
                    $("#addModal input").prop("checked", false);
                    $("#addModal select").val("");
                    $("#addModal").find("input[name='fsResno']").val(fsRoomno);
                    $("#addModal").find("input[name='fsRoomname']").val(fsRoomname);
                    $('#addModal').modal({
                        show : true,
                        backdrop : 'static'
                    });
